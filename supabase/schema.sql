...
alter table public.products add column if not exists affiliate_network text;
alter table public.products add column if not exists commission_model text;
alter table public.products add column if not exists commission_value numeric;
alter table public.products add column if not exists commercial_priority integer not null default 0;
alter table public.products add column if not exists sponsored boolean not null default false;

create index if not exists products_commercial_priority_idx on public.products(commercial_priority);
...
drop policy if exists "public insert product clicks" on public.product_clicks;
create policy "public insert product clicks" on public.product_clicks
  for insert to anon, authenticated
  with check (event_type in ('affiliate_click','official_click'));

drop policy if exists "admin read product clicks" on public.product_clicks;
create policy "admin read product clicks" on public.product_clicks
