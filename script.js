let products=window.CP_PRODUCTS||[];const toastEl=document.getElementById('toast');let toastTimer;
...
async function loadProducts(){
  ...
  try{
    const c=window.CP_SUPABASE||{};
    if(c.url&&!c.url.includes('TU-PROYECTO')&&c.anonKey&&!c.anonKey.includes('TU_')&&window.supabase){
      const client=window.supabase.createClient(c.url,c.anonKey);
      const {data,error}=await client.from('products').select('*').eq('active',true).order('featured',{ascending:false}).order('commercial_priority',{ascending:false}).order('updated_at',{ascending:false});
      if(!error&&data&&data.length){
        products=data.map(p=>({...p,id:p.id,verified:p.verified_at||p.verified}));
        if(status)status.textContent=`● ${products.length} productos activos`;
      }else if(status){status.textContent='● Catálogo local';}
    }else if(status){status.textContent='● Catálogo local';}
  }catch(e){
    console.warn('Supabase fallback:',e);
    if(status)status.textContent='● Catálogo local';
  }
